...
// model — this is Steve's "operations" brain. Previously lived in
// mockAiEngine.js and was driven by a separate "akaza" assistant app;
// Steve now owns this entirely, since he's meant to be the one place in
// Campus that can actually do things, not just talk about them.
export async function getSteveOpsResponse({ message }) {
  ...
  if (/^(help|what can you do|commands?)\??$/.test(text)) {
    return {
      text:
        "Here's what's on my desk right now: open apps (\"open files\"), navigate to a folder by path " +
  ...
  if (pathMatch) {
    ...
    return { text: result.ok ? result.message : `Couldn't find that: ${result.message}` };
  }
  ...
  if (listMatch) {
    ...
    if (!result.ok) return { text: `Couldn't find that: ${result.message}` };
  ...
  if (renameMatch) {
    ...
    if (!target) return { text: `Couldn't find anything named "${from.trim()}".` };
  ...
  if (trashMatch) {
    ...
    if (!target) return { text: `Couldn't find anything named "${name}".` };
  ...
  if (text.includes("workspace") && (text.includes("stat") || text.includes("what's in") || text.includes("summar"))) {
    ...
    return {
      text: result.ok
        ...
        : "Couldn't read your workspace stats right now.",
    };
  }
  ...
  if (/^(who (are|r) you|what are you)\??$/.test(text)) {
    return {
      text:
        "I'm Steve — I run point on Campus. I greet new folks, keep score on what you've explored, and " +
        "handle the actual operations: opening apps, moving files, searching your workspace, all of it.",
    };
  }

  return {
    text:
      "Didn't catch a command there. Try things like \"open Files\", \"open folder Documents/Projects\", " +
